Feature: what a suite cannot be run as written
  ...
  Scenario: a scenario that arranges after it has acted is refused before anything runs
    ...
    When I run "--config .workspaces/broken/atf.yaml run"
    Then the result field "exit_code" is "2"
    And the result field "output" contains "has already acted"
    And the result field "output" contains "Every Given comes first"
  ...
  @run
  Scenario: a field the resource never declared is refused where it is written
    Given the workspace "mistyped"
    When I run "--config .workspaces/mistyped/atf.yaml run" as "typo"
    Then the typo field "exit_code" is "2"
    And the typo field "output" contains "Note declares path, text"
    And the typo field "output" contains "also sets txt"
