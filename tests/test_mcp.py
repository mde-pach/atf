...
from atf.mcp import Session, unavailable
...
def session(tmp_path, monkeypatch) -> Session:
    ...
    yield Session(Cockpit("dev"))
    set_cockpit(None)


def test_an_environment_nobody_declared_is_refused_rather_than_quietly_replaced(session):
    ...
    with pytest.raises(ValueError, match="unknown environment 'staging'"):
        session.describe(environment="staging")


def test_running_against_an_environment_nobody_allowed_to_change_is_refused(session):
    """Running provisions, so `run` is held to `mutable_envs` exactly as the composer's Try it is."""
    answered = session.run(scenario="anything", environment="locked")
    assert answered["ran"] is False
    assert "mutable_envs" in answered["problems"][0]


def test_running_a_scenario_this_suite_has_not_got_names_the_ones_it_has(session):
    answered = session.run(scenario="no::such-thing")
    assert answered["ran"] is False
    assert "no scenario 'no::such-thing' here" in answered["problems"][0]


def test_running_a_draft_that_is_not_a_scenario_yet_reports_why_instead_of_running_it(session):
    """The refusal arrives before anything is provisioned, which is what makes it a refusal."""
    answered = session.run(title="", rows=[{"keyword": "when", "pattern": "I invent a step"}])
    assert answered["ran"] is False
...
def test_with_the_sdk_installed_the_server_offers_exactly_the_three_verbs(session):
    """Three tools, and they do not grow when the vocabulary does — that is the whole design."""
    import asyncio

    from atf.mcp import server

    built = server(session.cockpit)
